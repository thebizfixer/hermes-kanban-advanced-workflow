...
def _check_git_freshness(plan_id: str, project_root: Path) -> tuple[bool, str]:
    ...
    if attestation.exists():
        age_seconds = time.time() - attestation.stat().st_mtime
        if age_seconds > 300:  # 5 minutes — stale
            issues.append(f"  attestation: {attestation} ({age_seconds:.0f}s old)")
        # else: fresh attestation (< 5 min) — skip, it was generated by this session's pre_dispatch_gate
