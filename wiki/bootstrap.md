...
10. **Cron scripts (files only)** — `auto_unblock.sh`, `board_keeper.sh`, `token_tracker.py`, `dashboard_server_keepalive.py`, `scripts/lib/resolve_notify_deliver.sh`, `scripts/lib/hermes_notify_deliver.py` → `$HERMES_HOME/scripts/` (and `lib/`). Does **not** create Hermes cron **jobs** — those are per-plan at **execute/handoff** (`kanban_handoff.py` → `provision_kanban_crons.sh --create`), verified at decomposition.

   **Exception — keepalive cron:** The dashboard sidecar keepalive (`kanban-dashboard-keepalive`) IS created during init because the sidecar starts immediately and needs crash recovery from the start. The cron references the bare filename `dashboard_server_keepalive.py` — the Hermes cron runner resolves it against `$HERMES_HOME/scripts/`. The script MUST be copied into `$HERMES_HOME/scripts/` before cron creation (init does this via `materialize_hermes_scripts()`). If the cron is created with a relative plugin path, it breaks silently — the error is only visible in `jobs.json`.
...
13. **Hermes kanban config** — `hermes config set kanban.auto_decompose false` and `kanban.dispatch_stale_timeout_seconds 14400` (stale reclaim safety net; rationale: [`dispatch-stale-timeout.md`](../plugin/data/references/dispatch-stale-timeout.md)). Advisory if `hermes config` fails — same pattern as `auto_decompose`. **Invariant check:** after patching `BLOCK_RECURRENCE_LIMIT`, verifies `BLOCK_RECURRENCE_LIMIT >= kanban.failure_limit` from `config.yaml`. Emits a non-blocking WARN when violated (exit 0) — operators decide whether to fix.
