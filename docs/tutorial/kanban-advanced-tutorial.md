...
The agent will draft a plan (often in an IDE-native directory such as `.cursor/plans/`). During **Harden**, it is copied to the canonical path `.hermes/kanban/plans/kanban-health.plan.md`.
...
> "Harden the plan at `.hermes/kanban/plans/kanban-health.plan.md`" (or link the draft path — Harden copies it to `.hermes/kanban/plans/` if needed)
...
> "Start an orchestrator session (`hermes -p kanban-advanced-orchestrator chat`), then say: 'Harden the plan at `.hermes/kanban/plans/kanban-health.plan.md`' (or your draft path). The orchestrator copies the plan to `.hermes/kanban/plans/` if needed, then verifies everything against your codebase and adds any missing test coverage. When it prompts for review, you can accept or ask for changes — say 'Revise the test plan to include integration tests' if something's missing. This loop can run as many times as you want. When you're happy, we'll move to Optimize."
