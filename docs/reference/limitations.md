...
| Cron job creation at init | Plugin can't call `cronjob` during init/bootstrap | Init materializes **scripts only**; per-plan jobs via `provision_kanban_crons.sh --create --workdir <repo-root>` at decomposition. Use `--headless` when no gateway: manual `auto_unblock.sh` loop. Cron cwd defaults to gateway start dir without `--workdir`. |
...
| Agent binary verification | Plugin lists only on-PATH supported commands at init/dashboard; warns on contested shared names (e.g. `agent`). Does not identify which product owns a name via `--version` or repair symlinks | On-PATH picker + `check_coding_agent_cli.py` headless smoke at preflight; see [coding-agents.md](coding-agents.md) § Binary name collisions |
