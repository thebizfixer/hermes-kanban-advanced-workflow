...
description: How to write implementation plans optimized for Kanban decomposition â file-level granularity, section structure, agent-prompt blocks, ordinal card body templates, and dependency graphs. Distinguishes Harden (WHAT â content completeness, sanity check, gap closure) from Optimize (HOW â formatting for decomposition, agent blocks, budgets, graphs).
version: 5.3.0
metadata:
  hermes:
...
Plans must use **markup-safe placeholders** â many renderers treat `<word>` as HTML-like tags and break preview or hide sections. **Never use angle-bracket placeholders** (`<plan_id>`, `<card2-branch>`, `<command>`). Use **`{placeholder}`** or backticks instead.
...
**Quick rules:**
- Agent blocks: single ` ```agent ` fence only â no outer ` ````markdown ` wrapper.
- Non-trivial cards: include `Spec:`, `Call-sites:`, `Forbidden:`, `Acceptance:`, `Self-audit:` per Â§Card body agent-prompt template.
...
**Self-check before Optimize attestation:** `grep -nE '<[A-Za-z_/]' <plan>.md` â fix placeholder matches (allow `<=` / `>=` only).
...
| **Execute** | `"Execute the plan"` | **Orchestrator profile only.** Decompose into kanban cards, dispatch workers. Non-orchestrator profiles prefer the **board-mediated handoff**: `python3 scripts/kanban_handoff.py --plan <plan.md>` creates one hardened handoff card the dispatcher runs under the orchestrator profile (idempotent; checks its own preconditions). **Fallback only** (no gateway): start a new orchestrator session â `/profile` does **not** switch â via `hermes -p orchestrator chat`. See `plugin/data/references/profile-switching.md`. | Decomposed board |
...
1. **Agent prompt blocks present** â Every code-generation workstream section includes an executable `agent -p` fenced code block. Verify: `grep -c '```agent' <plan>.md` equals the number of code workstreams. Plans without these blocks are rejected at the orchestrator's attestation gate (P002).
2. **No `--model` in card bodies** â Agent-prompt blocks must not include `--model` when the assignee profile has model preferences in `config.yaml` (i.e., `model.default` is set). The profile determines the model; the card body specifies only the task. Card body policy P005 blocks cards that override profile model config at dispatch.
3. **Iteration budget estimated** â Each card's happy-path operations must fit within **35 turns**, leaving 55 for debugging within the 90-turn default. Formula: `(functions Ã 3) + (test_runs Ã 2) + (consumer_checks Ã 2) + (import_fixes Ã 2) + 2 buffer`. Cards exceeding 35 estimated turns **must be split** regardless of line count. Code relocation offers no exemption â 19 function extractions burn ~57 turns before tests even run.
4. **Files:/Mode: lines present** â Every workstream header declares `Files:` (which files the card touches) and `Mode:` (modify-only, create-only, or any). The evaluation chain verifies these at steps E001 and E002.
5. **Commit granularity aligned** â One atomic commit per kanban card. Commit messages pre-written in each section's agent block (`Commit:` line).
6. **Dependency graph drawn** â ASCII-art dependency graph with parent-child link table, parallel dispatch waves, and same-provider serialization noted. The orchestrator uses this to run `hermes kanban link`. A prose "implementation order" sentence is not sufficient â draw the graph.
7. **Card order finalized, then labeled** â **Arrange first, label second.** Use the dependency graph to lock dispatch order (gate â holistic fixes â parallel waves â tests â audit). Only after order is final, write the `## Kanban optimization` section with sequential ordinals `#### Card 1`, `#### Card 2`, â¦ `#### Card N`. Do not label while still reordering. Forbidden in the optimization section: letter labels (A, B, C), draft workstream names (`Workstream 2a`, `WS3`), or non-contiguous numbers (Card 3, Card 6, Card 2). Draft-phase `###` sections may keep descriptive names; **Kanban optimization** is the canonical dispatch sequence. See Â§Kanban optimization section.
8. **Same-provider staggering planned** â Cards on the same provider serialized via parent-child links (or documented as auto-serialized by the dispatcher). See the provider-strategy wiki page for multi-provider fan-out and fallback configuration.
9. **Line budget computed** â Total net line changes estimated across all cards. No single card exceeds 200 net lines. The `line_budget` field in the frontmatter summarizes the plan total.
10. **Card granularity verified** â No card bundles more than 2 distinct file-level changes. Cards with 3+ files are split regardless of line count. Same-file cards that touch the same file are serialized via parent-child links.
...
18. **Executive summary documentation-ready** â For plans that serve double-duty as implementation plans AND documentation artifacts (linked from `docs/` or referenced by other plans), the executive summary must be self-contained: a reader who never opens the evidence matrix must understand the problem, root causes, what's already fixed, what each phase delivers, and target metrics. Anti-pattern: a flat severity table. Required structure: (a) one-sentence opening, (b) root causes table with fix-complexity column, (c) already-shipped items, (d) remediation-at-a-glance phase table, (e) key performance targets in before/after format, (f) closing line with blast radius.
19. **Acceptance surface audit** â Each YAML todo â list every deliverable (code path, test file, operator script, deploy step). If more than one surface, split the todo or add numbered `Acceptance:` items in the agent block.
20. **Call-site audit** â For any "wire X to Y" todo, `rg` all callers; agent block lists every `Call-sites: path:symbol` or names a shared resolver both paths must use.
21. **Files completeness** â For UX/helper changes, `rg` the symbol across the repo; all subscriber-facing call sites appear in `Files:` or an explicit follow-up card.
22. **Verification taxonomy** â `verification-local` (pytest only) vs `verification-deploy` (operator + deploy); never mark deploy todos `completed` on merge to staging alone.
23. **Same-file graph** â Mark cards that touch the same production file; planner sets `wave_parent` chain (not parallel gate-only siblings) for those files.
24. **Multi-parent cap** â Test/doc cards: max **2** production parents unless `Mode: read-only`; otherwise split fixtures by file.
25. **Spec precision** â Non-trivial agent blocks include `Spec:` + `Anchor:` + `Examples:`; `Contracts:` in `## Kanban optimization` pins every `Call-sites:` symbol; ban vague integration verbs unless followed by a concrete operation (see `plan-file-format.md`).
26. **Plan memory seed** â At end of Optimize, write `acceptance_matrix` (or equivalent) into plan frontmatter or `.hermes/kanban/memory/{plan_id}.json` for decompose to expand.
27. **Markup-safe placeholders** â No `<word>` slots anywhere in the plan file; use `{word}` per Â§Plan file format.

### Frontend decomposition (route micro-cards)

When a workstream changes subscriber-visible layout or motion:

1. **`{feature}-ux-helpers`** â pure functions/hooks with unit tests (no route shell edits).
2. **`{feature}-route-wiring`** â wire helpers into the route shell (`Files:` includes `{ui_stack.page_glob}`).
3. **`{feature}-route-layout`** â DOM order + `Acceptance (layout):` grep bullets (line order before/after slot anchors).
4. **`integration-verify`** â `Type: verification-local` card after the route-layout group; runs pytest + `kanban_layout_acceptance.sh` paths.
5. **Deploy smoke** â `Type: verification-deploy` with `Deploy:` line; requires `.hermes/kanban/card-attestations/{plan_id}-{card_key}.json` before archive.

Declare `Surface-slots:` in the plan and mirror host paths in overlay `ui_stack` (`frontend-neutrality.md`). `verify_optimization.sh` checks 19â21 enforce presentation acceptance at Optimize time.
...
The trigger phrase table in the interaction model, the adoption protocol, and all script invocations must follow this convention. Historical plan files and changelogs are exempt â they're immutable records of what was built.
...
| One stubborn outcome lane on a board | kanban-advanced + `--goal` on **0â2** cards after Harden (`plugin/data/references/goal-card-selection.md`) |

Requires **Hermes â¥ 0.16.0** for `--goal` on `hermes kanban create`.

## Plan optimization (summary)

The Harden and Optimize checklists above replace the older single "12-item checklist." See Â§Harden checklist for content-completeness verification (anchor points, edge cases, redundant changes, auto-research) and Â§Optimize checklist for kanban-execution-readiness verification (agent blocks, iteration budgets, dependency graphs, Files:/Mode:).
...
2. **Clear section-per-change.** Each fix or feature gets its own `###` section with:
  - File path(s) â where the change belongs and how it integrates
  - Implementation approach â what's needed to fulfill the requirement
  - **`agent -p` fenced block** â the exact command the worker executes (see template below)
  - Test strategy
  - Edge cases
3. **File-level granularity.** Never describe a change that spans 3+ files in one section â split it.
4. **Explicit dependencies.** If section B depends on section A, say so. The orchestrator uses this for parent-child linking.

Build what's needed first. Flag what's wanted as optional. When in doubt, implement the simplest solution that fulfills the requirements â and ask before adding anything beyond them.
...
Forbidden: {never-tier â paths, deps, no signature changes elsewhere}
Acceptance:
...
Do NOT push to ${working_branch} â commit to worktree branch only."
```
...
> **Model name requirement:** If the valid model set is unknown, omit `--model` entirely â the CLI auto-selects.
...
**What is NOT Wanted?** {failure modes â maps to error codes E001-E006}
**How is it NOT Wanted?** {sad paths â maps to recovery actions}

**Where does it NOT belong?** {restricted paths â maps to E002/E009/E011}
**How does it NOT belong there?** {boundary enforcement â maps to card policy P001-P004}

**When is it NOT received?** {environmental failures â maps to E007/E008/E012}
**How will it NOT be received?** {governance infra failures â maps to A001-A003/E013}
...
| Board / plan validation | Failures â warnings | Warnings pass with review | Warnings â block |
...
**Rule:** Confirm `pwd` resolves to a native filesystem path before running any kanban operation. If the working copy is on a translated or mounted path, clone it to a native location first. This check is part of the preflight checklist (see `kanban-advanced:kanban-preflight` Â§ Filesystem coherence check).

## Commit cadence

After completing every section of a plan, commit and push before starting the next. This rule ensures that a runtime crash, gateway timeout, or agent restart loses at most one section of work â not the entire plan run.
...
|---|---|---|---|---|
| `preflight` hard-fail | Low | BLOCKING | Fix environment, re-run preflight.sh | No â manual fix required |
| `preflight` degraded | Medium | DEGRADED | Warn, proceed with reduced parallelism | Yes â 1 retry with fresh session |
| Agent auth failure | Low | BLOCKING | Check CURSOR_API_KEY / auth.json | No â re-auth required |
| Agent timeout (>900s) | Medium | BLOCKING | Split card into smaller chunks | Yes â 1 retry before block |
| Test failure post-commit | Medium | BLOCKING | Revert, re-plan, re-execute | No â fix code first |
| Same-file collision (no parent link) | Low | BLOCKING | Add `hermes kanban link` in plan | No â plan edit required |
| Token tracking failure | Low | BLOCKING | Verify `scripts/token_tracker.py` import | No â fix imports first |
| Gateway notification unreachable | Low | DEGRADED | Log to file, continue silently | Yes â 2 retries with backoff |
| Evaluation chain missing (E013) | Low | BLOCKING | Restore kanban_evaluation_chain.py | No â restore file first |
| Attestation stale (A002) | Medium | BLOCKING | Re-run preflight + attestation | Yes â 1 retry |

### Gating rules

- **BLOCKING impact** â Halt decomposition. Notify user with specific fix required. Do NOT auto-retry.
- **DEGRADED impact** â Log warning, reduce parallelism or skip non-critical steps, continue. Auto-retry once if pattern supports it.
...
1. **Count additions** â new lines the card will introduce.
2. **Count deletions** â existing lines the card will remove.
3. **Count rewrites** â lines modified in-place (count as 1 addition + 1 deletion).
4. **Estimate iterations** â count distinct operations: function extractions (~3 turns each), test runs (~2 turns), consumer verifications (~2 turns), import fixes (~2 turns), commits (~1 turn). The happy path should consume no more than **40 turns**, leaving 50 for debugging within the default **90-turn budget**.

Net change = additions + deletions + rewrites.

| Net lines | Iterations (est.) | Action |
|---|---|---|
| â¤ 50 | â¤ 15 | Preferred â review for bundling with sibling changes |
| 51â100 | 15â30 | Normal â proceed |
| 101â200 | 30â50 | Warning â verify granularity; split if touching >2 files |
| > 200 | > 50 | **FLAG â must split.** Either line count or iteration estimate exceeds safe bounds. |

> **Code relocation is NOT exempt from splitting.** Moving 300 lines of existing code (add+del=600, net=30) is still a large card. The agent must read, understand, copy, verify imports, remove, re-export, test, and commit â each step consumes iterations. A 19-function extraction with full test suite easily burns 60+ happy-path turns and exhausts the 90-turn budget on any failure. Split relocation cards the same way you split greenfield cards. See `plugin/data/references/iteration-budget-case-study.md` for a worked example (WS9: 19 functions, 72 happy-path turns, exhausted 90-turn budget).
...
(Single `agent` fence â see Â§Card body agent-prompt template; use {placeholders}, not angle brackets.)
```

## Kanban optimization section (mandatory output)

After the Optimize checklist passes, append (or rewrite) a **`## Kanban optimization`** section. `kanban_decompose.py` reads **only** this section â draft `###` headings elsewhere are not dispatch ordinals.

**Workflow: arrange first, label second**

1. **Arrange** â Order cards by the dependency graph (gate â holistic fixes â parallel waves â tests â audit). Resolve merges and splits before naming.
2. **Label** â Renumber in that order as `#### Card 1 â {title}`, `#### Card 2 â {title}`, â¦ through `#### Card N`. Integers only, contiguous from 1, no gaps, no out-of-order appearance in the file.
3. **Cross-reference** â Agent blocks and `wave_parent` / `ordinal_parent` fields use `Card N`, not draft names (`Workstream 2a`, `WS3`, letter labels).
...
#### Card 1 â Gate (manual)
plan_id: â¦
wave: 1
â¦

#### Card 2 â {first implementation card}
plan_id: â¦
files:
  - path/to/file.py
mode: modify-only
...
#### Card 3 â {next card in dispatch order}
â¦
```

Run `bash hermes-kanban-advanced-workflow/scripts/verify_optimization.sh --plan <plan>.md` â check **15** enforces sequential `Card N` labeling.
...
- **Too coarse.** "Fix all extraction issues" â 3+ cards. Split by file.
- **Too fine.** "Change variable name from x to y" â merge with sibling changes to same file.
- **Missing dependencies.** If section B reads a function section A creates, mark it as dependent.
- **No file paths.** "Modify the orchestrator" â the decomposer can't route it. Always include concrete paths.
...
- **Code relocation is not free.** Moving functions between files still consumes agent iterations â reading, copying, verifying imports, removing, re-exporting, testing, committing. Estimate iterations per operation (see Line Budget Analysis) and split accordingly. A 19-function extraction is easily 3+ cards.
- **Plan lacks holistic vs surgical classification.** When a plan lists all fixes at equal priority without classifying them as holistic (global, few files) vs surgical (complex, cross-cutting), the decomposition can't optimize ordering â holistic fixes should dispatch first because they unblock surgical work and reduce blast radius. Add a classification column to the signal map before optimizing for Kanban.
...
- **Angle-bracket placeholders.** Using `<plan_id>`, `<command>`, or `<card2-branch>` in plan prose â breaks markup preview; use `{plan_id}` per `plan-file-format.md`.
...
- `plugin/data/references/plan-file-format.md` â markup-safe placeholders, `Spec:` contract block, acceptance surfaces, fence rules
- `plugin/data/references/plan-anchor-verification-pitfalls.md` â common inaccuracy patterns when verifying plan claims against the codebase
- `plugin/data/references/worker-actionability-audit.md` â per-section actionability checklist before decomposition
- `plugin/data/references/single-coherent-filesystem.md` â filesystem coherence and commit cadence incident analysis
- `plugin/data/references/documentation-style.md` â system-agnostic paths, CLI Agent vs vendor binary naming, wiki table formatting, user-authored prose preservation
- `plugin/data/references/documentation-sanity-check.md` â stale reference detection, code fence integrity, table formatting, package tree maintenance
- `plugin/data/references/readme-formatting-pitfalls.md` â URL-encoded HTML artifacts, HTML entities, broken code fences, triple blanks, user-authored prose preservation
- `plugin/data/references/vanilla-kanban-known-issues.md` â upstream Hermes Agent kanban bugs mapped to structural workarounds (dependency gating, workspace isolation, dispatcher resilience, root card anti-patterns)
- `plugin/data/references/iteration-budget-case-study.md` â worked example: WS9 19-function extraction exhausted 90-turn budget; how to calculate operation counts and split correctly
- `plugin/data/references/governance-sad-path-audit.md` â full flowchart trace of every transition with 23 sad paths, governance coverage assessment, and prioritized gaps (kanban-advanced:kanban-orchestrator reference)
- `plugin/data/references/plan-hardening-checklist.md` â 11-item first-pass hardening checklist (Critical â Important â Nice-to-have) + redundant change detection pattern; runs between sanity check and optimization
- `plugin/data/references/phase-transition-hardening.md` â re-verifying line numbers, fleshing out placeholder workstreams, dependency graphs, and verification gates when reactivating a deferred plan phase
- `plugin/data/references/plan-hardening-methodology.md` â tier-gated hardening pass (Critical â Important â Nice-to-have) after a sanity check; verification grep suite; before/after report template
- `plugin/data/references/dependency-graph-format.md` (kanban-advanced:kanban-orchestrator) â ASCII-art dependency graph format for parent-child link planning
- **Wiki: provider-strategy** â multi-provider fan-out, rate-limit prevention, fallback configuration (for same-provider staggering decisions in checklist item 9)
- **Wiki: Why NOT Kanban** (README Â§ Why NOT Kanban) â when to skip the workflow entirely (scope-appropriateness gate before planning begins)
