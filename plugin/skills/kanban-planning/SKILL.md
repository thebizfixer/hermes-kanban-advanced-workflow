...
| **Execute** | `"Execute the plan"` | **Orchestrator profile only.** Decompose into kanban cards, dispatch workers. Non-orchestrator profiles must refuse with actionable steps: run `hermes profile list` (show `*` = active), explain that `/profile` does **not** switch profiles, then tell the user to start **`hermes -p orchestrator chat`** (or `orchestrator chat` / `hermes profile use orchestrator` + `hermes chat`) and repeat the trigger. See `references/profile-switching.md`. | Decomposed board |
