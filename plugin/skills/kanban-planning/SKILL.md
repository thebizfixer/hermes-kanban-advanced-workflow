...
- **Docs cards run last (or reconcile against HEAD).** Documentation workstreams that describe shipped behavior must execute after implementation waves. Running a docs card before its implementation cards guarantees staleness — the docs describe pre-change state. If docs must dispatch early (e.g., operator-authored plan overview), include explicit `Reconcile against HEAD after wave N` acceptance in the card body and a `Type: acceptance-verify` follow-up card gating on the last implementation wave. See `verify_optimization.sh` inverted-graph WARN (enforced in Card 6). Example anti-pattern: clockwork-v2 ran Card 1 (docs/EXTRACTION.md) before Cards 2–5 (implementation), guaranteeing the extraction doc shipped stale.
